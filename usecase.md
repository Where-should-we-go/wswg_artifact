...
> **문서 버전**: v2.1 (9테이블 문서모델·모임 모델·발자취 지도 격상·인기 여행지 추천 반영) / **개정**: 2026-06-23
...
| 액터 | 설명 |
| :--- | :--- |
| **게스트 (Guest)** | 비로그인 방문자. 인기 여행지 추천·관광 정보 열람, 소셜 로그인 가능 |
...
─────────────────   ────────────────────────   ──────────────────────────  ────────────────────
B0 인기 여행지 추천  →  C1 모임 → D1/D2 여행생성   E1 현장 미디어 업로드        자동 완료 판정
B2/B3 관광지 탐색    C2 초대 → D3/D4 편집        (= D4 편집의 시간적 변주)    E3 대표 추억 선정
...
| 화면 | URL | 담는 UC | 비고 |
|------|-----|---------|------|
| **S1 랜딩** | `/` | **B0** + 서비스 소개 | Hero + 인기 여행지 추천 + 시작하기 |
...
> **신규 화면**: S9 모임 관리. **확장 화면**: S1(인기 여행지 추천), S6(4뷰·미디어), S8(핵심).
